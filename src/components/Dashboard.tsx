  ...
  return (
    <div className="space-y-6 pb-12 animate-fade-in">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-slate-900 p-6 text-white relative overflow-hidden flex flex-col justify-between rounded-2xl shadow-md">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500">Acompanhamento de fluxo de caixa projetado e saldo real de cada conta.</p>
            </div>
          </div>
        ...
        <div className="bg-white/80 backdrop-blur-md p-6 rounded-2xl border border-gray-100 shadow-sm flex flex-col justify-between min-h-[350px]">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-base font-bold text-gray-900 flex items-center gap-2">
              ...
              Transações Recentes no Supabase
            </h3>
          ...
          <div className="flex-1 flex flex-col justify-center space-y-2 py-2">
            {loading ? (
              <p className="text-center text-xs text-gray-400">Carregando transações...</p>
            ) : transactions.length === 0 ? (
              <p className="text-center text-xs text-gray-400">Nenhuma transação encontrada.</p>
            ) : (
              transactions.slice(0, 4).map((tx) => (
                <div key={tx.id} className="flex items-center justify-between bg-gray-50 p-3 rounded-xl text-xs">
                  <div>
                    <span className="font-semibold text-gray-800">{tx.title}</span>
                ...
                {transactions
                  ...
                  .map((tx) => (
                    <div key={tx.id} className="flex justify-between items-center text-sm border-b border-gray-50 pb-2">
                      <span className="text-gray-700 font-medium">{tx.category}</span>
                      <span className="text-rose-600 font-bold">R$ {Number(tx.amount).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</span>
                    </div>
                  ))}
              </div>
            )}
